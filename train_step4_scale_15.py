...
def get_args():
    ...
    parser.add_argument('--fname', default='cifar_model_step4_scale_15', type=str)
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            scale_rate = (1.5 / robust_loss).cuda().type(torch.cuda.FloatTensor)
            Indicator_above = (scale_rate <= 1).cuda().type(torch.cuda.FloatTensor)
            Indicator_below = (scale_rate > 1).cuda().type(torch.cuda.FloatTensor)
            loss_scale_rate = scale_rate * Indicator_below + Indicator_above
            robust_loss = robust_loss.mul(loss_scale_rate)
            ...
            ignore_x += Indicator_below.sum().item()
