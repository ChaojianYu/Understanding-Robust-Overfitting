...
def get_args():
    ...
    parser.add_argument('--fname', default='cifar_model_step3_remove_transformed_0_15', type=str)
...
def main():
    ...
    if args.model == 'PreActResNet18':
        ...
        model_tmp = PreActResNet18()
    elif args.model == 'WideResNet':
        model = WideResNet(34, 10, widen_factor=args.width_factor, dropRate=0.0)
        model_tmp = WideResNet(34, 10, widen_factor=args.width_factor, dropRate=0.0)
    else:
        raise ValueError("Unknown model")

    model = nn.DataParallel(model).cuda()
    model_tmp = nn.DataParallel(model_tmp).cuda()
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            if epoch >= 100:
                model_tmp.load_state_dict(torch.load(os.path.join(args.fname, f'model_99.pth')))
                delta_tmp = attack_pgd(model_tmp, X, y, epsilon, pgd_alpha, args.attack_iters, args.restarts, args.norm)
                delta_tmp = delta_tmp.detach()
                X_adv_tmp = normalize(torch.clamp(X + delta_tmp[:X.size(0)], min=lower_limit, max=upper_limit))
                robust_output_tmp = model_tmp(X_adv_tmp)

            robust_loss = nn.CrossEntropyLoss(reduce=False)(robust_output, y)
            if epoch < 100:
                Indicator_ignore = (robust_loss < 1.5).cuda().type(torch.cuda.FloatTensor)
                Indicator_reserve = 1 - Indicator_ignore
            else:
                robust_loss_tmp = nn.CrossEntropyLoss(reduce=False)(robust_output_tmp, y)
                Indicator_ignore_tmp = (robust_loss_tmp < 1.5).cuda().type(torch.cuda.FloatTensor)
                Indicator_ignore = (robust_loss < 1.5).cuda().type(torch.cuda.FloatTensor)
                Indicator_reserve = 1 - Indicator_ignore
                Indicator_reserve = Indicator_reserve + Indicator_ignore_tmp
                Indicator_reserve = (Indicator_reserve > 0.5).cuda().type(torch.cuda.FloatTensor)
            robust_loss = robust_loss.mul(Indicator_reserve)
            ...
            ignore_x += Indicator_ignore.sum().item()
        ...
        if not args.eval:
            logger.info('%d \t %.1f \t \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f \t \t %.4f \t \t %.4f \t %.4f \t %.4f \t \t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f',
